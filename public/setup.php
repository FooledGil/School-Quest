 ...
 * Akses via browser: https://sk0011jb6j.skipper.my.id/school_quest_deploy/public/setup.php
 */
...
// Cari file SQL di beberapa kemungkinan folder
$possible_sql_paths = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($db_pass)) {
        ...
        $error = 'File school_quest_backup.sql tidak ditemukan! Pastikan file backup sql sudah di-upload ke hosting.';
    } else {
        try {
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type="text"], input[type="password"] { width: 100%; padding: 10px 14px; border: 1.5px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; }
        .btn { width: 100%; background: #2563eb; color: white; padding: 12px; border: none; border-radius: 8px; font-weight: 600; font-size: 15px; cursor: pointer; margin-top: 10px; }
        ...
        <?php else: ?>
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="db_pass" placeholder="Paste password database dari Dashboard Skipper" required autofocus>
                </div>
