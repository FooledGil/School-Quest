...
export default function Login() {
    ...
    return (
        <AuthLayout>
            ...
            {/* Tab Switcher */}
            <div className="grid grid-cols-2 mb-5 sm:mb-6 bg-[#0f1118] p-1 rounded-lg border border-[#242836]">
                <button
                    type="button"
                    onClick={() => switchTab('student')}
                    className={`py-2 text-[11px] sm:text-xs font-mono font-bold tracking-wider uppercase transition-all duration-200 rounded-md cursor-pointer ${
                        tab === 'student'
            ...
            <form onSubmit={submit} className="space-y-4 tab-fade-in" key={tab}>
                ...
                <button
                    type="submit"
                    disabled={processing}
                    className="w-full py-3 sm:py-3.5 px-4 rounded-lg bg-[#f59e0b] hover:bg-[#d97706] text-black font-extrabold font-mono tracking-wider text-xs sm:text-sm uppercase transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-amber-500/20 active:translate-y-0 shadow-md flex items-center justify-center gap-2 mt-5 sm:mt-6 cursor-pointer"
                >
                    {processing ? (
                        'ENTERING REALM...'
                    ) : (
            ...
            <div className="bg-[#0d1017] border border-[#222634] rounded-lg p-3 text-[11px] sm:text-xs font-mono text-slate-300 mt-5 transition-all duration-200 hover:border-amber-500/30">
                <div className="text-[10px] font-bold text-amber-500 tracking-wider uppercase flex items-center gap-1.5 mb-1">
                    <InformationCircleIcon className="w-3.5 h-3.5 text-amber-500 animate-pulse shrink-0" />
                    SCROLL OF ACCESS
                </div>
                {tab === 'student' ? (
                    <p className="text-slate-300 break-words">
                        Demo: <span className="text-white font-bold">NISN 0012345001</span> | Pass: <span className="text-white font-bold">password</span>
                    </p>
                ) : (
                    <p className="text-slate-300 break-words">
                        Demo: <span className="text-white font-bold">admin@schoolquest.id</span> | Pass: <span className="text-white font-bold">admin123</span>
                    </p>
                )}
            </div>
        </AuthLayout>
