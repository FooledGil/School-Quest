...
export default function Dashboard({ user: propUser, schedules = [], stats = {}, recentQuests = [], achievements = [] }) {
    ...
    return (
        <StudentLayout user={{ ...user, avatar, rank_name: rankName, next_level_exp: nextLevelExp }}>
            ...
            <div ref={pageRef}>
                {/* Hero Section */}
                <div ref={heroRef} className="glass-card p-6 md:p-8 mb-8 relative overflow-hidden bg-slate-900/60 border-l-4 border-l-blue-500 shadow-xl">
                    <div className="flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
                        <div className="relative group">
                            <div className="w-20 h-20 md:w-28 md:h-28 rounded-2xl p-0.5 bg-gradient-to-br from-blue-500 to-indigo-600 shadow-md transform transition-transform group-hover:scale-105">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            <div className="flex flex-col md:flex-row md:items-center justify-between mb-2">
                                <div>
                                    <h1 className="text-2xl md:text-3xl font-extrabold text-white mb-1 tracking-tight">{user.name}</h1>
                                    <p className="text-slate-400 font-medium text-sm">Kelas {user.class || 'Siswa'} • <span className="text-amber-400 font-bold">{rankName}</span></p>
                                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <section>
                            ...
                            {recentQuests.length > 0 ? (
                                <div ref={questsRef} className="grid sm:grid-cols-2 gap-4">
                        ...
                        <section className="glass-card p-6">
                            <h2 className="text-lg font-bold text-white mb-6">Pencapaian (Achievements)</h2>
                            <div ref={achievementsRef} className="flex flex-wrap justify-around gap-4">
                    ...
                    <div className="space-y-8">
                        {/* Schedule */}
                        <section ref={scheduleRef} className="glass-card p-6 border-t-2 border-t-blue-500">
                            <h2 className="text-lg font-bold text-white mb-6 text-center">Jadwal Hari Ini</h2>
                            <ScheduleTimeline schedule={schedules.map(s => ({
                                subject: s.subject?.name || 'Pelajaran',
                                room: s.class,
                                startTime: s.time_start?.substring(0, 5) || '07:30',
                                endTime: s.time_end?.substring(0, 9) || '09:00',
                                icon: s.subject?.icon || '📚'
                            }))} />
                        </section>
                    </div>
