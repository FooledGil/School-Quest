...
export default function AuthLayout({ children }) {
    return (
        <main className="min-h-screen min-h-dvh flex flex-col justify-center items-center relative overflow-hidden bg-[#0a0e17] text-gray-200 font-body px-3.5 sm:px-4 py-6 sm:py-10" role="main">
            ...
            <div className="z-10 w-full max-w-md auth-card-anim">
                ...
                <div className="text-center mb-5 sm:mb-6 flex flex-col items-center">
                    ...
                    <p className="text-slate-400 font-mono text-[11px] sm:text-xs tracking-widest uppercase flex items-center justify-center gap-1">
                        Leveling up<span className="inline-block w-1.5 h-1.5 bg-slate-500 mb-0.5 animate-pulse" />education
                    </p>
                </div>

                {/* Main Auth Card Container */}
                <div className="bg-[#161922] border border-[#262b3a] shadow-2xl rounded-xl p-5 sm:p-7 md:p-8 relative transition-all duration-300">
                    {children}
                </div>

                {/* Footer Links */}
                <div className="flex justify-between items-center text-[10px] sm:text-[11px] font-mono tracking-wider text-slate-500 mt-5 px-1 uppercase">
                    <button type="button" onClick={() => alert('Fitur ini belum tersedia untuk mode demo.')} className="hover:text-slate-300 transition-colors cursor-pointer">
                        FORGOT KEY?
                    </button>
                    <div>
                        <span>NEW RECRUIT? </span>
                        <button type="button" onClick={() => alert('Mode pendaftaran akun dikelola oleh Admin Sekolah.')} className="text-white font-bold hover:underline transition-colors cursor-pointer">
                            JOIN GUILD
                        </button>
                    </div>
                </div>
